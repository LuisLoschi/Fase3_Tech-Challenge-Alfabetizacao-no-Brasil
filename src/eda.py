...
def formatar_milhar(valor: float) -> str:
    """Separador de milhar, para rótulo de eixo e texto corrido."""
...
def auc_univariada(y, escore) -> float:
    ...
    Feature que sozinha fica em 0,5 raramente vira sinal dentro de um GBM, 
    e descobrir isso custa um ranking de postos em vez de uma tarde de busca 
    de hiperparâmetro.
...
def medir_auc_por_porte_da_escola(
    ...
    Testa a hipótese de que a taxa da escola é fraca por ruído amostral: 
    se fosse, escola grande, onde a taxa tem erro-padrão pequeno,
...
def medir_auc_das_features(coorte: pd.DataFrame, features: list[str], alvo: str = "alfabetizado") -> pd.DataFrame:
    ...
    # 0,45 separa tanto quanto 0,55; só muda o sinal. compara na mesma escala.
