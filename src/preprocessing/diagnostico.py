...
def medir_dicionario(dados: pd.DataFrame, features: list[str] | None = None) -> pd.DataFrame:
    ...
    A AUC univariada é calculada sobre o alvo invertido (1 = alfabetizado) para
    poder ser comparada com a EDA. A cobertura é calculada sobre o dataset final 
    (não sobre a Gold), para que o dicionário reflita o que de fato entra no modelo.
    """
...
def comparar_pareado(replica: pd.DataFrame, base: str, alternativo: str, k: int = 5) -> dict:
    """Delta pareado por fold com o t corrigido de Nadeau-Bengio.
